...
var FailurePolicyReason = /** @class */ (function () {
    ...
    FailurePolicyReason.prototype.toError = function () {
        switch (this.data.kind) {
            case 'failure':
                return new errors_1.SDKExecutionError(FailurePolicyReason.failureToString(this.data.failure), [
                    "At " + new Date(this.data.failure.time).toISOString(),
                    this.prefixMessages.join(': '),
                ], []);
            case 'policy':
                return new errors_1.SDKExecutionError("Failure policy aborted with reason:' " + this.data.reason, [this.prefixMessages.join(': ')], ['Check that the failure policy is correctly configured']);
        }
    };
