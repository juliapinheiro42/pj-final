...
function mergeProfileDefaults(document, profileName, payload) {
    ...
    if (typeof targetedProfile === 'string') {
        ...
        if (targetedProfile.defaults) {
            ...
            defaults = variables_1.mergeVariables(variables_1.castToNonPrimitive(targetedProfile.defaults) || {}, variables_1.castToNonPrimitive(payload) || {});
...
function mergeProfile(document, profileName, payload) {
    var _a;
    var _b, _c, _d;
    // if specified profile is not found
    ...
    if (typeof payload === 'string') {
        var isShorthandAvailable = typeof targetedProfile === 'string' ||
            (variables_1.isEmptyRecord((_b = targetedProfile.defaults) !== null && _b !== void 0 ? _b : {}) &&
                variables_1.isEmptyRecord((_c = targetedProfile.providers) !== null && _c !== void 0 ? _c : {}));
        ...
        if (ast_1.isFileURIString(payload)) {
            if (isShorthandAvailable) {
                document.profiles[profileName] = schema_1.composeFileURI(payload);
                return true;
            }
            document.profiles[profileName] = __assign({ file: schema_1.trimFileURI(payload) }, commonProperties);
            return true;
        }
        ...
        throw new errors_1.UnexpectedError('Invalid string payload format');
    }
    ...
    if (typeof targetedProfile === 'string') {
        ...
        if (targetedProfile.defaults) {
            if (!payload.defaults) {
                ...
                //Merge existing with new
                defaults = variables_1.mergeVariables(variables_1.castToNonPrimitive(targetedProfile.defaults) || {}, variables_1.castToNonPrimitive(payload.defaults) || {});
            }
        }
    ...
    if (typeof targetedProfile === 'string') {
        ...
        Object.entries((_d = payload.providers) !== null && _d !== void 0 ? _d : {}).forEach(function (_a) {
            var _b = __read(_a, 2), providerName = _b[0], entry = _b[1];
            return mergeProfileProvider(document, profileName, providerName, entry);
        });
 ...
 */
function ensureProfileWithProviders(document, profileName) {
    var changed = false;
    if (document.profiles === undefined) {
        document.profiles = {};
        changed = true;
    ...
    if (typeof profile === 'string') {
        document.profiles[profileName] = normalize_1.normalizeProfileSettings(document.profiles[profileName], []);
        changed = true;
    }
...
function mergeProfileProvider(document, profileName, providerName, payload) {
    var _a;
    var _b, _c, _d;
    var _e = __read(ensureProfileWithProviders(document, profileName), 2), _ = _e[0], targetProfile = _e[1];
    void _;
    ...
    if (!profileProvider || !((_c = targetProfile.providers) === null || _c === void 0 ? void 0 : _c[providerName])) {
    ...
    if (typeof payload === 'string') {
        if (typeof profileProvider === 'string' ||
            variables_1.isEmptyRecord((_d = profileProvider.defaults) !== null && _d !== void 0 ? _d : {})) {
            targetProfile.providers[providerName] = schema_1.composeFileURI(payload);
            targetProfile.priority = resolvePriorityAddition(targetProfile.priority, providerName);
            return true;
        }
        targetProfile.providers[providerName] = {
            file: schema_1.trimFileURI(payload),
    ...
    if (typeof profileProvider === 'string') {
        ...
        if (profileProvider.defaults && payload.defaults) {
            //Change
            //Merge existing with new
            defaults = variables_1.mergeVariables(variables_1.castToNonPrimitive(profileProvider.defaults) || {}, variables_1.castToNonPrimitive(payload.defaults) || {});
        }
...
function swapProfileProviderVariant(document, profileName, providerName, variant) {
    var _a = __read(ensureProfileWithProviders(document, profileName), 2), _ = _a[0], targetProfile = _a[1];
    void _;
    var changed = false;
    ...
    if (variant.kind === 'local') {
        if (typeof targetProfileProvider === 'string') {
            ...
            changed = schema_1.composeFileURI(variant.file) === targetProfileProvider;
            targetProfileProvider = schema_1.composeFileURI(variant.file);
        }
...
function mergeProvider(document, providerName, payload) {
    var _a, _b, _c, _d;
    ...
    if (typeof payload === 'string') {
        ...
        if (ast_1.isFileURIString(payload)) {
            if (isShorthandAvailable) {
                document.providers[providerName] = schema_1.composeFileURI(payload);
            }
            else {
...
function swapProviderVariant(document, providerName, variant) {
    ...
    if (variant.kind === 'local') {
        if (typeof targetProvider === 'string') {
            changed = schema_1.composeFileURI(variant.file) !== targetProvider;
            targetProvider = schema_1.composeFileURI(variant.file);
        }
...
function setPriority(document, profileName, providersSortedByPriority) {
    var _a, _b;
    if (document.profiles === undefined) {
        document.profiles = {};
    }
    ...
    //if specified profile has shorthand notation
    if (typeof targetedProfile === 'string') {
        document.profiles[profileName] = targetedProfile = normalize_1.normalizeProfileSettings(targetedProfile, Object.keys((_a = document.providers) !== null && _a !== void 0 ? _a : {}));
    }
